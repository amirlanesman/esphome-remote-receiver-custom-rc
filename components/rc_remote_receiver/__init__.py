...
from esphome.components.remote_receiver import (
    RemoteReceiverComponent,
    validate_tolerance,
    )
from .. import rc_remote_base
...
AUTO_LOAD = ["remote_base", "rc_remote_base"]
# DEPENDENCIES=["remote_receiver"]

MULTI_CONF = True
CONFIG_SCHEMA = rc_remote_base.validate_triggers(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(RemoteReceiverComponent),
            cv.Required(CONF_PIN): cv.All(pins.internal_gpio_input_pin_schema),
            cv.Optional(CONF_DUMP, default=[]): rc_remote_base.validate_dumpers,
...
async def to_code(config):
    ...
    dumpers = await rc_remote_base.build_dumpers(config[CONF_DUMP])
    for dumper in dumpers:
        cg.add(var.register_dumper(dumper))

    triggers = await rc_remote_base.build_triggers(config)
